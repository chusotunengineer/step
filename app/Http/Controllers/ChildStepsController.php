...
class ChildStepsController extends Controller
{
    ...
    public function create(Request $request){
        ...
        for($i=1; $i<=$posts['count']; $i++){
            ...
            if(!empty($posts[$name]) && !empty($posts[$content])){
                ...
                $child_step = ChildStep::create($record);
                $count++;
            }
        }
    ...
    public function edit(){
        ...
        $child_step = ChildStep::where('parent_id', $id)->exists();
        if(!$child_step){
    ...
    public function editAjax(){
        $child_step = ChildStep::where('parent_id', $_GET['id'])->get();
        return $child_step;
    }
    ...
    public function update(Request $request){
        ...
        for($i=1; $i<=$posts['count']; $i++){
            $child_step = ChildStep::where([['parent_id', $posts['parent_id']],['order', $i]])->first();
